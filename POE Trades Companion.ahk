...
;			Is transparent and click-through when there is no trade on queue
	...
	if ( mode = "CREATE" ) {
		...
		ProgramValues.IBStyle_TabMsg := IBStyle_TabMsg
		ProgramValues.IBStyle_TabJoined := IBStyle_TabJoined
...
; - - - - - Tabs
		Loop %maxTabsRendered% {
			...
			if !ImageButton.Create(hTabJoined%A_Index%, IBStyle_TabJoined*)
				MsgBox, 0, ImageButton Error Tab, % ImageButton.LastError
			...
			if !ImageButton.Create(hTabMsg%A_Index%, IBStyle_TabMsg*)
				MsgBox, 0, ImageButton Error Tab, % ImageButton.LastError
